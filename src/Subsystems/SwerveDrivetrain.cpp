...
SwerveDrivetrain::SwerveDrivetrain()
	...
	m_kinematics(
		Translation2D(RobotParameters::wheelTrack / 2.0, RobotParameters::wheelBase / 2.0),
		Translation2DRobotParameters::wheelTrack / 2.0, -RobotParameters::wheelBase / 2.0),
		Translation2D(-RobotParameters::wheelTrack / 2.0, -RobotParameters::wheelBase / 2.0),
		Translation2D(-RobotParameters::wheelTrack / 2.0, RobotParameters::wheelBase / 2.0)),
    m_swerveDrivePose(
    		Pose2D(Translation2D(0, 0), Rotation2D::fromDegrees(0)),
			RobotParameters::k_wheelTrack,
			RobotParameters::wheelBase,
...
void SwerveDrivetrain::driveOpenLoopControl(double percentVelX, double percentVelY, double percentYawRate) {
	...
	double robotYawRate = (RobotParameters::k_maxSpeed / pow(pow(RobotParameters::k_wheelTrack / 2.0, 2) + pow(RobotParameters::k_wheelBase / 2.0), 0.5)) * 180.0 / M_PI) * percentYawRate;
...
void SwerveDrivetrain::driveClosedLoopControl(
	...
	if(maxWheelAccel > RobotParameters::k_maxAccel) {
		frWheelAccel.scaleBy(1.0 / maxWheelAccel);
		brWheelAccel.scaleBy(1.0 / maxWheelAccel);
		blWheelAccel.scaleBy(1.0 / maxWheelAccel);
		flWheelAccel.scaleBy(1.0 / maxWheelAccel);
	}
    ...
    double frWheelAngVel = frWheelVel.rotateBy(frWheelYaw.inverse()).getY() / RobotParameters::k_wheelRad * 180.0 / M_PI;
    double brWheelAngVel = brWheelVel.rotateBy(brWheelYaw.inverse()).getY() / RobotParameters::k_wheelRad * 180.0 / M_PI;
    double blWheelAngVel = blWheelVel.rotateBy(blWheelYaw.inverse()).getY() / RobotParameters::k_wheelRad * 180.0 / M_PI;
    double flWheelAngVel = flWheelVel.rotateBy(flWheelYaw.inverse()).getY() / RobotParameters::k_wheelRad * 180.0 / M_PI;
    double frWheelAngAccel = frWheelAccel.rotateBy(frWheelYaw.inverse()).getY() / RobotParameters::k_wheelRad * 180.0 / M_PI;
    double brWheelAngAccel = brWheelAccel.rotateBy(brWheelYaw.inverse()).getY() / RobotParameters::k_wheelRad * 180.0 / M_PI;
    double blWheelAngAccel = blWheelAccel.rotateBy(blWheelYaw.inverse()).getY() / RobotParameters::k_wheelRad * 180.0 / M_PI;
    double flWheelAngAccel = flWheelAccel.rotateBy(flWheelYaw.inverse()).getY() / RobotParameters::k_wheelRad * 180.0 / M_PI;
...
//	bool isHighGear = m_pShifter->Get();
	...
	if(isHighGear) {
		m_pFRDriveMotor->setTicksPerRev(RobotParameters::k_grayhillEncoderTicksPerRev * RobotParameters::k_driveMotorToEncoderGearRatioHigh);
		m_pBRDriveMotor->setTicksPerRev(RobotParameters::k_grayhillEncoderTicksPerRev * RobotParameters::k_driveMotorToEncoderGearRatioHigh);
		m_pBLDriveMotor->setTicksPerRev(RobotParameters::k_grayhillEncoderTicksPerRev * RobotParameters::k_driveMotorToEncoderGearRatioHigh);
		m_pFLDriveMotor->setTicksPerRev(RobotParameters::k_grayhillEncoderTicksPerRev * RobotParameters::k_driveMotorToEncoderGearRatioHigh);
	}
	else {
		m_pFRDriveMotor->setTicksPerRev(RobotParameters::k_grayhillEncoderTicksPerRev * RobotParameters::k_driveMotorToEncoderGearRatioLow);
		m_pBRDriveMotor->setTicksPerRev(RobotParameters::k_grayhillEncoderTicksPerRev * RobotParameters::k_driveMotorToEncoderGearRatioLow);
		m_pBLDriveMotor->setTicksPerRev(RobotParameters::k_grayhillEncoderTicksPerRev * RobotParameters::k_driveMotorToEncoderGearRatioLow);
		m_pFLDriveMotor->setTicksPerRev(RobotParameters::k_grayhillEncoderTicksPerRev * RobotParameters::k_driveMotorToEncoderGearRatioLow);
	}
